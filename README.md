...
1. **Java JDK 17 or later** (runs and compiles the Java application)
...
1. Go to **https://adoptium.net** and download a Java JDK 17 or later installer.
...
You should see Java 17 or a later version.

---

### Step 2 — Install Maven

1. Download and install **Apache Maven** for Windows.
2. Add Maven's `bin` directory to your Windows PATH if the installer does not do so automatically.
3. Open a new PowerShell and verify:
...
You should see the Maven version and the Java version being used.
...
This builds the Java application and creates the runnable JAR in the `target` directory.

> **Troubleshooting:**
> - `mvn: command not found` → install Maven and make sure it is added to PATH
> - `java: command not found` → install Java JDK and make sure JAVA_HOME/PATH is configured
> - If Maven is not installed, you can compile the source with Java 17 directly using `javac`.
...
```powershell
java -jar target/vectordb-java.jar
```
...
├── src/main/java/com/vectordb/     ← Java backend
│   ├── Main.java                   ← REST API and web server
│   ├── VectorDB.java               ← Unified vector database
│   ├── DocumentDB.java             ← Document vector database
│   ├── HNSW.java                   ← HNSW index
│   ├── KDTree.java                 ← KD-Tree index
│   ├── BruteForce.java             ← Exact brute-force search
│   ├── Distance.java               ← Distance metrics
...
| `mvn: command not found` | Install Maven and add it to Windows PATH |
...
Then edit `OllamaClient.java` where `genModel` is set:
```java
